...
export default function FavouriteProperty() {
  ...
  useEffect(() => {
    if (!guestId) return; // Prevents request with missing guestId
    const token = localStorage.getItem("token");
    if (!token) return; // do not send request if no auth token 
    async function fetchFavouriteStatus() {
      try {
        const res = await axios.get(
          `https://be-airbnc-zw86.onrender.com/api/properties/${id}/favourite`,
          {
  ...
  async function handleFavouriteClick() {
    try {
      if (!isFavourited) {
        ...
        await axios.delete(
          `https://be-airbnc-zw86.onrender.com/api/favourites/${favouriteId}`
        );
        setIsFavourited(false);
        setFavouriteId(null);
      }
